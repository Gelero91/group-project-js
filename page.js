const movieCards = [
  ...
  let filmIndex = parseInt(params.get("filmIndex"));
  
  function page(filmIndex) {
      let name = movieCards[filmIndex].name;
      let date = movieCards[filmIndex].date;
      let poster = movieCards[filmIndex].poster;
      let synopsis = movieCards[filmIndex].synopsis;
  
      const filmInfo = `
        <div>
        <img src="${poster}" alt="${name}, ${date}, poster"/>
        <div class="info">
          <h2>${name}</h2>
          <h3>${date}</h3>
          <p>${synopsis}</p>
        ...
        <div">
            <div class="btn"><a href="./index.html">Retour</a></div>
        </div>
        </div>
        `;
  
      document.getElementById("filmInfo").innerHTML += filmInfo;}
